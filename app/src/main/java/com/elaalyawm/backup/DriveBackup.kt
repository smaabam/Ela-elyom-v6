...
class DriveBackup(private val context: Context, private val db: AppDatabase) {
    ...
    suspend fun upload(account: Account): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            // Check for existing backup and delete it to keep only one latest backup
            val existingFiles = service.files().list().setSpaces("appDataFolder")
                .setQ("name = 'ela-al-yawm-backup.json' and trashed = false")
                .execute().files
            existingFiles.forEach { service.files().delete(it.id).execute() }
